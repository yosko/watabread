...
/**
 * Manager implementing basic BREAD operations for entities stored in an SQL database:
 *  - browse: getList(), count()
 *  - read: get(), getBy()
 *  - edit: update()
 *  - add: add()
 *  - delete: delete()
 */
abstract class BreadBaseManager extends SqlManager
{
     ...
     */
    protected function applyFilters(SqlGenerator $qry, $context = 'list')
    {
        return $qry;
    }
     ...
     * @return SqlGenerator          Query object modified
     */
    protected function settingsForGetAndList(SqlGenerator $qry, bool $isCount)
    {
        return $qry;
    }
    ...
    protected function setPrimaryKeyFilter(SqlGenerator $qry, array $primary, bool $useAlias = true): SqlGenerator
    {
        ...
        foreach ($primary as $key => $param) {
            ...
            $qry->where($str, $key, $param['value'], $param['type']);
        }
        return $qry;
    }
     ...
     * @param mixed $primary primary key (this default version uses a single int value for a field named id)
     ...
     * @return object the data row/object
     */
    public function getBy(string $filter, $value)
    {
        $qry = $this->prepareGetStatement();
     ...
     */
    final protected function prepareGetStatement()
    {
        $qry = $this->newSqlGenerator();
        $qry->select($this->tableName, $this->tableAlias, array($this->tableAlias . '.*'));
        $qry = $this->settingsForGetAndList($qry, false);
     ...
     */
    protected function postGet(object $values)
    {
        return $values;
    }
